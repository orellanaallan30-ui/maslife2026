    ...
    <div className="biomech-report-modal fixed inset-0 z-[200] bg-slate-950 overflow-y-auto">
      ...
      <div className="sticky top-0 z-10 bg-slate-950/95 backdrop-blur border-b border-slate-800 px-4 lg:px-10 py-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.35em] text-amber-400">Informe Biomecánico</p>
              ...
              {(report.metricas || []).map((m, i) => (
                <div key={i} className="bg-slate-900 rounded-3xl border border-slate-800 p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{m.zona || 'Postura'}</p>
                  ...
                  <span className="inline-block mt-3 text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-full"
              ...
              <div key={i} className="bg-slate-900 rounded-3xl border border-slate-800 p-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-950 rounded-2xl p-4 border border-slate-800">
                    <p className="text-[9px] font-black uppercase tracking-widest text-sky-400 mb-1">Izquierda</p>
                    <p className="text-sm font-bold text-slate-200">{s.izquierda}</p>
                  </div>
                  <div className="bg-slate-950 rounded-2xl p-4 border border-slate-800">
                    <p className="text-[9px] font-black uppercase tracking-widest text-violet-400 mb-1">Derecha</p>
            ...
            {romEntries.map(r => (
              <div key={r.key} className="bg-slate-900 rounded-3xl border border-slate-800 p-5">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <p className="text-xl font-black" style={{ color: r.color }}>{r.val}° <span className="text-[10px] text-slate-500 font-bold">/ {r.normal}°</span></p>
                </div>
                <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(100, r.pct)}%`, background: r.color }} />
                </div>
                <p className="text-[10px] font-bold text-slate-500 mt-1">{r.pct}% del rango normal</p>
              </div>
            ))}
            {romEntries.length === 0 && (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <div key={c.l} className="bg-slate-900 rounded-3xl border border-slate-800 p-5 text-center">
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{c.l}</p>
            ...
            {discrepVal !== null && !Number.isNaN(discrepVal) && (
              <div className="bg-slate-900 rounded-3xl border border-slate-800 p-5 text-center col-span-2 md:col-span-1">
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Discrepancia MMII</p>
          ...
          <div className="space-y-4 max-w-4xl">
            {report.impresion_global && (
              <div className="bg-slate-900 rounded-3xl border border-slate-800 p-6">
                <p className="text-[10px] font-black uppercase tracking-widest text-amber-400 mb-2">Impresión Biomecánica Global</p>
            ...
            {report.diagnostico && (
              <div className="bg-slate-900 rounded-3xl border border-amber-400/30 p-6">
                <p className="text-[10px] font-black uppercase tracking-widest text-amber-400 mb-2">Diagnóstico Postural Kinesiológico</p>
            ...
            {(report.objetivos?.length || 0) > 0 && (
              <div className="bg-slate-900 rounded-3xl border border-slate-800 p-6">
                <p className="text-[10px] font-black uppercase tracking-widest text-teal-400 mb-3">Objetivos de Tratamiento</p>
                <ol className="space-y-2">
                  {report.objetivos!.map((o, i) => (
                    <li key={i} className="flex gap-3 text-sm text-slate-200">
                      <span className="w-5 h-5 rounded-full bg-teal-400/15 text-teal-400 text-[10px] font-black flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                      {o}
                    </li>
                  ))}
                </ol>
            ...
            {(report.plan?.length || 0) > 0 && (
              <div className="bg-slate-900 rounded-3xl border border-slate-800 p-6">
                <p className="text-[10px] font-black uppercase tracking-widest text-sky-400 mb-3">Plan Kinesiológico Sugerido</p>
